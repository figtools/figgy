...
resource "aws_iam_role" "bastion_user_role" {
  count = local.bastion_enabled ? length(local.role_types) : 0
...
resource "aws_iam_user" "users" {
  count = local.bastion_enabled && local.bastion_account_number == var.aws_account_id ? length(local.bastion_users) : 0
...
resource "aws_iam_group" "groups" {
  count = local.bastion_enabled && local.bastion_account_number == var.aws_account_id ? length(local.role_types) : 0
...
resource "aws_iam_policy" "read_figgy_configs" {
  count = local.bastion_enabled && local.bastion_account_number == var.aws_account_id ? 1 : 0
